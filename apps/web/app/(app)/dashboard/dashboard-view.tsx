...
export function DashboardView({ hasApiKey }: { hasApiKey: boolean }) {
  const [mode] = useMode();
  const engineRunning = false; // engine runtime not implemented yet
  ...
  return (
    <div className="max-w-[1440px] mx-auto space-y-6">
      ...
      {!hasApiKey || !engineRunning ? (
        <OnboardingChecklist mode={mode} hasApiKey={hasApiKey} engineRunning={engineRunning} />
      ) : null}
      ...
      <p className="text-[11.5px] text-ink-muted text-center pt-2">
        Strategy engine runtime is not yet attached to this account. Past performance
...
function OnboardingChecklist({
  ...
  const steps: {
    ...
    { id: 1, done: true, label: "Account created", hint: "Welcome." },
    {
      ...
      hint: hasApiKey
        ? "Key connected. You can manage it from API keys."
        : mode === "demo"
        ? "Use a Demo Trading key — toggle Demo Trading on bybit.com first."
        : "Use a mainnet key with Read + Trade permissions only.",
      cta: hasApiKey
        ? { href: "/keys", label: "Manage" }
        : { href: "/keys", label: "Connect key" },
    },
    {
      id: 3,
      done: engineRunning,
      label: "Start strategy engine",
      hint: hasApiKey
        ? "Engine runtime is not yet shipped. Coming next — runs the H.D.L.X rules against live market data."
        : "Engine starts once a key is connected.",
            ...
            <div className="flex-1 min-w-0">
              <div
                className={cn(
                  ...
                  s.done ? "text-ink-secondary" : "text-ink-primary",
                )}
              >
                {s.label}
              </div>
